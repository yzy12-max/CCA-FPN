...
class RandomFlip:
    ...
    def __init__(self, flip_ratio=None, direction='horizontal'):
        ...
        valid_directions = ['horizontal', 'vertical', 'diagonal']   # 水平,垂直和对角线
...
class MotionBlur:
    ...
    def motion_blur(self, img, img_shape):
        ...
        """
        if img_shape[1] <= 750:  # 如果图像的宽小于等于750,模糊核的大小是在(10-20)之间选取
            kernel_size = int(rand.sample(range(10, 20), 1)[0])
        elif img_shape[1] < 2000 and img_shape[1] > 750: # 如果图像的宽750-2000,模糊核的大小是在(20-60)之间选取
            kernel_size = int(rand.sample(range(20, 60), 1)[0])
        elif img_shape[1] >= 2000:
            kernel_size = int(rand.sample(range(100, 140), 1)[0])   # 如果图像的宽>=2000,模糊核的大小是在(100-1400)之间选取
        kernel_motion_blur = np.zeros((kernel_size, kernel_size))   # 初始化模糊核
        kernel_motion_blur[int((kernel_size - 1) / 2), :] = np.ones(kernel_size)  # 将中间的一行数据全变为1
        kernel_motion_blur = kernel_motion_blur / kernel_size   # 除以核的边长大小,简单的均值模糊核
    ...
    def _mosaic_transform(self, results):
        ...
        if len(results['img'].shape) == 3:
            mosaic_img = np.full(
                ...
                self.pad_val,
                dtype=results['img'].dtype)     # 使用self.pad_val填充
        else:
            mosaic_img = np.full(
        ...
        for i, loc in enumerate(loc_strs):
            if loc == 'top_left':
                results_patch = copy.deepcopy(results)  # 拷贝一份数据
            else:
                results_patch = copy.deepcopy(results['mix_results'][i - 1])

            img_i = results_patch['img']    #
            h_i, w_i = img_i.shape[:2]      # 图像高宽
            # keep_ratio resize
            scale_ratio_i = min(self.img_scale[0] / h_i,
                                self.img_scale[1] / w_i)    # 取最小的比列
            img_i = mmcv.imresize(
                img_i, (int(w_i * scale_ratio_i), int(h_i * scale_ratio_i)))    # 先RESIZE

            # compute the combine parameters
            paste_coord, crop_coord = self._mosaic_combine(
                loc, center_position, img_i.shape[:2][::-1])    #img_i.shape[:2][::-1]取高宽
            x1_p, y1_p, x2_p, y2_p = paste_coord
            x1_c, y1_c, x2_c, y2_c = crop_coord

            # crop and paste image
            mosaic_img[y1_p:y2_p, x1_p:x2_p] = img_i[y1_c:y2_c, x1_c:x2_c]  #裁减粘贴到对应mosaic范围里

            # adjust coordinate
            gt_bboxes_i = results_patch['gt_bboxes']    # 有四张图片,这里先取第一张的(左上角的)
        ...
        if len(mosaic_labels) > 0:
            ...
            mosaic_bboxes[:, 0::2] = np.clip(mosaic_bboxes[:, 0::2], 0,
                                             2 * self.img_scale[1])     # 查看是否超出范围,超出范围就裁减掉
            mosaic_bboxes[:, 1::2] = np.clip(mosaic_bboxes[:, 1::2], 0,
                                             2 * self.img_scale[0])
            mosaic_labels = np.concatenate(mosaic_labels, 0)

            if not self.skip_filter:        # 是否过滤掉一些不合适的(太小)
...
class RandomAffine:
    ...
    def __call__(self, results):
        img = results['img']    # 刚开始乘以了2,现在变换回去
        height = img.shape[0] + self.border[0] * 2  # self.border:(-1333//2,-800//2)
        width = img.shape[1] + self.border[1] * 2

        # Rotation
        rotation_degree = random.uniform(-self.max_rotate_degree,
                                         self.max_rotate_degree)
        rotation_matrix = self._get_rotation_matrix(rotation_degree)    # 通过离散余弦变换来变换角度
        ...
        img = cv2.warpPerspective(  # 对图片进行视觉转换
            img,
            warp_matrix,
            dsize=(width, height),
            borderValue=self.border_val)
        results['img'] = img    # 对图像进行变换
        results['img_shape'] = img.shape
        ...
        for key in results.get('bbox_fields', []):
            ...
            if num_bboxes:
                ...
                warp_points = warp_matrix @ points  # 对BBOX进行变换
    ...
    def _get_rotation_matrix(rotate_degrees):
        radian = math.radians(rotate_degrees)   # 方法将角度 x 从度数转换为弧度
