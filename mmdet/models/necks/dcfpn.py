...
class DCFPN(BaseModule):
    ...
    def __init__(self,
        ...
        super(DCFPN, self).__init__(init_cfg)
        ...
        for i in range(self.start_level, self.backbone_end_level): #(,4)
            l_conv = ConvModule(
                in_channels[i],
                out_channels,
                1,
                conv_cfg=conv_cfg,
                norm_cfg=norm_cfg if not self.no_norm_on_lateral else None,  #1X1卷集后是否使用归一化
                act_cfg=act_cfg,
                inplace=False)
            fpn_conv = ConvModule(
                out_channels,
                ...
                padding=1,
                conv_cfg=conv_cfg,
                norm_cfg=norm_cfg,                          #3X3卷集后是否使用归一化
                act_cfg=act_cfg,
                inplace=False)
            # up = Ups()
            up = DCCcnn1(256,kernel_size=3)
    ...
    def forward(self, inputs):
        ...
        laterals = [
            ...
            for i, lateral_conv in enumerate(self.lateral_convs) #[512,1024,2048]
        ]
        #
        # raw_laternals = [laterals[i].clone() for i in range(len(laterals))]
        used_backbone_levels = len(laterals)
        for i in range(used_backbone_levels - 1, 0, -1): #2,1 实验C5不进行上采样和C4融合会怎样，改为-2。1
        ...
        # ]
        outs = [
            self.ups[i](laterals[i]) for i in range(used_backbone_levels-1)   #
        ]
        outs.append(self.fpn_convs[2](laterals[-1]))
        # outs = [
        ...
        if self.num_outs > len(outs):
            ...
            if not self.add_extra_convs:
                for i in range(self.num_outs - used_backbone_levels):
                    outs.append(F.max_pool2d(outs[-1], 1, stride=2)) #maxpooling
