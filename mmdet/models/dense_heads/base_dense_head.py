...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def get_bboxes(self,
        ...
        if score_factors is None:   # 是质量分数,比如FCOS的中心损失
        ...
        num_levels = len(cls_scores)    # 层数

        featmap_sizes = [cls_scores[i].shape[-2:] for i in range(num_levels)]   # 获取每层特征图大小
        mlvl_priors = self.prior_generator.grid_priors(
            featmap_sizes,
            dtype=cls_scores[0].device,
            device=cls_scores[0].device)  #每层特征层点的生成,[prior,4]

        result_list = []

        for img_id in range(len(img_metas)):    # 遍历每张图片
            img_meta = img_metas[img_id]
            cls_score_list = select_single_mlvl(cls_scores, img_id) #将结果对应到原图上
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, score_factor, priors) in \
            ...
            if self.use_sigmoid_cls:
                scores = cls_score.sigmoid()    # 将分类分数进行sigmod
            else:
            ...
            results = filter_scores_and_topk(       # 先通过阈值0.05筛选,找到给定的TOP-K数量的scores, labels, keep_idxs ,filtered_results
                scores, cfg.score_thr, nms_pre,     # 经过三次筛选
