...
class CenterNetHead(BaseDenseHead, BBoxTestMixin):
    ...
    def loss(self,
        ...
        loss_center_heatmap = self.loss_center_heatmap(
            center_heatmap_pred, center_heatmap_target, avg_factor=avg_factor) #avg_factor:总的目标数目，也是正样本数
    ...
    def get_targets(self, gt_bboxes, gt_labels, feat_shape, img_shape):
        ...
        """
        img_h, img_w = img_shape[:2]        #pad 的大小
        bs, _, feat_h, feat_w = feat_shape

        width_ratio = float(feat_w / img_w)
        height_ratio = float(feat_h / img_h)
        #gt_bboxes[-1]表示最后一张图的gt_bboxes
        ...
        for batch_id in range(bs):
            ...
            center_x = (gt_bbox[:, [0]] + gt_bbox[:, [2]]) * width_ratio / 2  #乘以width_ratio，是将目标的box缩放到128x128的尺度上
            center_y = (gt_bbox[:, [1]] + gt_bbox[:, [3]]) * height_ratio / 2
            gt_centers = torch.cat((center_x, center_y), dim=1)

            for j, ct in enumerate(gt_centers):
                ctx_int, cty_int = ct.int()  #求box的中心点坐标并取整，设为point
                ctx, cty = ct
                ...
                gen_gaussian_target(center_heatmap_target[batch_id, ind],
                                    [ctx_int, cty_int], radius) #在heatmap图上，以point为圆心，半径为R填充高斯函数计算值
        ...
        avg_factor = max(1, center_heatmap_target.eq(1).sum()) #进行逐元素的比较，若相同位置的两个元素相同，则返回True；若不同，返回False。
        target_result = dict(
