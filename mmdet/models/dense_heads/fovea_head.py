...
class FoveaHead(AnchorFreeHead):
    ...
    def loss(self,
        ...
        points = self.prior_generator.grid_priors(   # 对应原图范围的点
            featmap_sizes,
            dtype=bbox_preds[0].dtype,
            device=bbox_preds[0].device)
        num_imgs = cls_scores[0].size(0)
        flatten_cls_scores = [
            cls_score.permute(0, 2, 3, 1).reshape(-1, self.cls_out_channels) #[b,k,h,w]-[b,h,w,k]-[b*h*w,k]
            for cls_score in cls_scores  # 3 ge list
        ]
        flatten_bbox_preds = [
    ...
    def _get_target_single(self,
        ...
        for base_len, (lower_bound, upper_bound), stride, featmap_size, \
            ...
            labels = gt_labels_raw.new_zeros(featmap_size) + self.num_classes  # 创建一个featmap_size大小的labels，全部都赋值成背景类
            bbox_targets = gt_bboxes_raw.new(featmap_size[0], featmap_size[1], # 创建一个featmap_size大小的 box target,初始化所有的值填充为1
                                             4) + 1
            # scale assignment
            hit_indices = ((gt_areas >= lower_bound) &
                           (gt_areas <= upper_bound)).nonzero().flatten()  #按循环，逐渐分配层
            if len(hit_indices) == 0:
                label_list.append(labels)
                bbox_target_list.append(torch.log(bbox_targets))
                continue
            _, hit_index_order = torch.sort(-gt_areas[hit_indices])  # 把被分贝到当前的特征图的box，按面积从大到小进行排序
            hit_indices = hit_indices[hit_index_order]      #从大到小的面积索引
            gt_bboxes = gt_bboxes_raw[hit_indices, :] / stride    #除以相应的S，
            gt_labels = gt_labels_raw[hit_indices]    # labels和bboxes相匹配
            half_w = 0.5 * (gt_bboxes[:, 2] - gt_bboxes[:, 0])
            half_h = 0.5 * (gt_bboxes[:, 3] - gt_bboxes[:, 1])
            # valid fovea area: left, right, top, down。对应论文公式（4）花间得到的
            pos_left = torch.ceil(
                gt_bboxes[:, 0] + (1 - self.sigma) * half_w - 0.5).long(). \
                clamp(0, featmap_size[1] - 1)     #四舍五入
