...
class CenterPrior(nn.Module):
    ...
    def __init__(self,
        ...
        self.mean = nn.Parameter(torch.zeros(num_classes, 2))   #超参数
    ...
    def forward(self, anchor_points_list, gt_bboxes, labels,
        ...
        for slvl_points, stride in zip(anchor_points_list, self.strides):
            ...
            instance_center = self.mean[labels][None]   # m*label，学习
            # instance_sigma has shape (1, num_gt, 2)
            instance_sigma = self.sigma[labels][None]   # sigma*label
            # distance has shape (num_points, num_gt, 2)
            distance = (((single_level_points - gt_center) / float(stride) -
                         instance_center)**2)       #高斯中心的公式
            center_prior = torch.exp(-distance /
                                     (2 * instance_sigma**2)).prod(dim=-1)  #函数计算，给定每个点到中心点的高斯分布
            center_prior_list.append(center_prior)
        center_prior_weights = torch.cat(center_prior_list, dim=0) #中心点的权重就是

        if self.force_topk:     #这个用来干嘛阿
        ...
        center_prior_weights[~inside_gt_bbox_mask] = 0  #抚养本权重为0
...
class AutoAssignHead(FCOSHead):
    ...
    def get_pos_loss_single(self, cls_score, objectness, reg_loss, gt_labels,
        ...
        p_loc = torch.exp(-reg_loss)   #将位置LOSS与分类损失简单的联合起来
        ...
        confidence_weight = torch.exp(p_pos * 3)    #表示某个anchor为正样本的概率值，正样本的权重，3是调节放大倍数
        p_pos_weight = (confidence_weight * center_prior_weights) / (
            (confidence_weight * center_prior_weights).sum(
                0, keepdim=True)).clamp(min=EPS)        #公式：正样本的权重
        reweighted_p_pos = (p_pos * p_pos_weight).sum(0) #加权
    ...
    def get_neg_loss_single(self, cls_score, objectness, gt_labels, ious,
        ...
        p_neg_weight = torch.ones_like(joint_conf) #抚养本初始化权重为1
        if num_gts > 0:
            ...
            temp_weight = (1 / (1 - ious[foreground_idxs]).clamp_(EPS))     #抚养本权重跟正样本的IOU有关
            #正样本的loss会把正样本的 r e g regreg 学习的很好，而负样本的reg 一直不学习就渐渐没落了
            def normalize(x):
    ...
    def loss(self,
        ...
        inside_gt_bbox_mask_list, bbox_targets_list = self.get_targets(
            all_level_points, gt_bboxes)    #换了个inside_gt_bbox_mask_list：就是框内样本（都是正的）的索引
                                            #bbox_targets_list：这个没有去掉抚养本，就是全部的样本ltrb
        ...
        for gt_bboxe, gt_label, inside_gt_bbox_mask in zip(
            ...
            center_prior_weight, inside_gt_bbox_mask = \
                self.center_prior(all_level_points, gt_bboxe, gt_label,
                                  inside_gt_bbox_mask)
            center_prior_weight_list.append(center_prior_weight)  #给定中心正负样本权重
            temp_inside_gt_bbox_mask_list.append(inside_gt_bbox_mask)
        inside_gt_bbox_mask_list = temp_inside_gt_bbox_mask_list
        mlvl_points = torch.cat(all_level_points, dim=0)
        bbox_preds = levels_to_images(bbox_preds)   #Concat multi-level feature maps by image
        cls_scores = levels_to_images(cls_scores)
        objectnesses = levels_to_images(objectnesses)  #是预测的前景还是背景的
        ...
        for bbox_pred, encoded_targets, inside_gt_bbox_mask in zip(
            ...
            decoded_bbox_preds = self.bbox_coder.decode(
                expand_mlvl_points, expand_bbox_pred) #ltrb--box
            decoded_target_preds = self.bbox_coder.decode(
                expand_mlvl_points, encoded_targets)    #ltrb--box ,为了计算IOU和进行GIOU LOSS计算
            ...
            loss_bbox = self.loss_bbox(
                ...
                weight=None,                #回归LOSS，这个LOSS只是为了给后面用
                reduction_override='none')
            reg_loss_list.append(loss_bbox.reshape(num_points, temp_num_gt))  #一张图上多个目标的LOSS

        cls_scores = [item.sigmoid() for item in cls_scores]  #都是后面对其进行sigmoid()
        ...
        pos_loss_list, = multi_apply(self.get_pos_loss_single, cls_scores,
                                     ...
                                     center_prior_weight_list)   #正样本LOSS
        ...
        for i in range(len(img_metas)):
            ...
            if inside_gt_bbox_mask_list[i].any(): #或操作，如果有一个为 True,则返回 True。
                center_loss.append(
                    len(gt_bboxes[i]) /
    ...
    def _get_target_single(self, gt_bboxes, points):
        ...
        xs = xs[:, None]
        ys = ys[:, None]
        left = xs - gt_bboxes[..., 0]       #不用分层，不用中心采样
        right = gt_bboxes[..., 2] - xs
        top = ys - gt_bboxes[..., 1]
        bottom = gt_bboxes[..., 3] - ys
        bbox_targets = torch.stack((left, top, right, bottom), -1)
        if num_gts:
            inside_gt_bbox_mask = bbox_targets.min(-1)[0] > 0   #ltrb>0样本在框内就是正样本
        else:
