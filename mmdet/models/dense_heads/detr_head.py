...
class DETRHead(AnchorFreeHead):
    ...
    def forward_single(self, x, img_metas):
        ...
        batch_size = x.size(0)
        input_img_h, input_img_w = img_metas[0]['batch_input_shape']    # 原始图像的宽高
        masks = x.new_ones((batch_size, input_img_h, input_img_w)) # 建立个MASK:[batch_size, input_img_h, input_img_w]
        for img_id in range(batch_size):    # [B,h,w]
            img_h, img_w, _ = img_metas[img_id]['img_shape']
            masks[img_id, :img_h, :img_w] = 0  #全部赋值为0
        # X:[B,C,H,W],这里的H,W和MASK里的h,w不同
        x = self.input_proj(x)  # 2048-256 [B,C,H,W]
        # interpolate masks to have the same spatial shape with x
        masks = F.interpolate( # 为什么步直接在上面就给和X的形状呢,有点不理解
            masks.unsqueeze(1), size=x.shape[-2:]).to(torch.bool).squeeze(1) # [B,C,H,W]
        # position encoding
        pos_embed = self.positional_encoding(masks)  # [bs, embed_dim, h, w]
        # outs_dec: [nb_dec, bs, num_query, embed_dim] # self.query_embedding.weight:预测的ENBDING
