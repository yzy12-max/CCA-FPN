...
class FNetHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        dcn_base_offset = np.stack([dcn_base_y, dcn_base_x], axis=1).reshape(
            (-1))  #
        self.dcn_base_offset = torch.tensor(dcn_base_offset).view(1, -1, 1, 1)  #应该是定义卷集和
    ...
    def _init_layers(self):   #和FCOS不同，看他的结构图
        ...
        self.reppoints_cls_conv = DeformConv2d(self.feat_channels,       #256
                                               self.point_feat_channels, #256 会产生256个通道OFFSET [b,18,h,w]
    ...
    def forward_single(self, x):
        ...
        pts_out_init = self.reppoints_pts_init_out(
            self.relu(self.reppoints_pts_init_conv(pts_feat)))  #得到的返回值是[b,18,h,w]可变形卷集的偏移量
        ...
        ) + self.gradient_mul * pts_out_init    #初始pts_out_init分支梯度乘上系数，目的是希望降低该分支的梯度权重，V2要约束它，不能乱偏移
        ...
        pts_out_refine = self.reppoints_pts_refine_out(                 #offset作为卷集核
    ...
    def _point_target_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            pos_proposals[pos_inds, :] = proposals[pos_inds, :]   #取正样本对应的侯选矿
    ...
    def get_targets(self,
        ...
        proposals_list = images_to_levels(all_proposals, num_level_proposals)  #一阶段的预测框作为侯选框
    ...
    def loss(self,
        ...
        for i_img, center in enumerate(center_list):
            ...
            for i_lvl in range(len(pts_preds_refine)):
                bbox_preds_init = self.points2bbox(     #将初始的修改成pts_preds_refine，用自己的匹配取计算LOSS
                    pts_preds_refine[i_lvl].detach())  #将一阶段九个点的偏移按要求转换成BOX，还存在偏移
                bbox_shift = bbox_preds_init * self.point_strides[i_lvl]  #还是
                bbox_center = torch.cat(
                    [center[i_lvl][:, :2], center[i_lvl][:, :2]], dim=1)  #一个样本坐标CAT成两个
                bbox.append(bbox_center +
                            bbox_shift[i_img].permute(1, 2, 0).reshape(-1, 4)) #坐标中心+偏移，转换成左上角和右下角的形式。
