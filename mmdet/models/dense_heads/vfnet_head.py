...
class VFNetHead(ATSSHead, FCOSHead):
    ...
    def __init__(self,
        ...
        self.dcn_kernel = int(np.sqrt(self.num_dconv_points))  #k = 3
        ...
        self.reg_denoms = [  #[64,128,256,512,INF]
            regress_range[-1] for regress_range in regress_ranges
        ]
        self.reg_denoms[-1] = self.reg_denoms[-2] * 2  #[64,128,256,512,1024]
        self.center_sampling = center_sampling
        self.center_sample_radius = center_sample_radius  # r
    ...
    def _init_layers(self):
        ...
        self.vfnet_reg_conv = ConvModule(
            ...
            bias=self.conv_bias)
        self.vfnet_reg = nn.Conv2d(self.feat_channels, 4, 3, padding=1)  #[256,4,3,1]
    ...
    def forward_single(self, x, scale, scale_refine, stride, reg_denom):
        ...
        if self.bbox_norm_type == 'reg_denom':
            bbox_pred = scale(                  #预测四个偏移梁（l,r,t,b）
                self.vfnet_reg(reg_feat_init)).float().exp() * reg_denom #single ,reg_denom=64比用STRIDE应该要更好
        ...
        dcn_offset = self.star_dcn_offset(bbox_pred, self.gradient_mul,
                                          stride).to(reg_feat.dtype)  #给定偏移值

        # refine the bbox_pred
        reg_feat = self.relu(self.vfnet_reg_refine_dconv(reg_feat, dcn_offset)) #[b,4,h,w]
        #增加注意力机制
        # sam = self.sam(reg_feat)
        # reg_feat_init = reg_feat_init*sam

        bbox_pred_refine = scale_refine(
            self.vfnet_reg_refine(reg_feat)).float().exp()    #没有乘以reg_denom
        bbox_pred_refine = bbox_pred_refine * bbox_pred.detach()
        #bbox_pred_refine = bbox_pred.detach()
    ...
    def star_dcn_offset(self, bbox_pred, gradient_mul, stride):
        ...
        x1 = bbox_pred_grad_mul[:, 0, :, :]    # l,r,t,b
        ...
        bbox_pred_grad_mul_offset[:, 17, :, :] = x2  # x2  #少了3，6，8，9，10，15，三个点
        dcn_offset = bbox_pred_grad_mul_offset - dcn_base_offset  #【2，18，100，152】-【1，18，1，1】
    ...
    def loss(self,
        ...
        all_level_points = self.fcos_prior_generator.grid_priors(
            featmap_sizes, bbox_preds[0].dtype, bbox_preds[0].device)    #获得所有层的样本点数
        labels, label_weights, bbox_targets, bbox_weights = self.get_targets(
            cls_scores, all_level_points, gt_bboxes, gt_labels, img_metas,  #给有划分样本的区域一个TRUE OR FLASE
            gt_bboxes_ignore)   #labels，bbox_targets定义和分配。分配到FPN各个特征层上
                                #bbox_targets放着的是【l,t,r,b】两张图片里的合并了
        num_imgs = cls_scores[0].size(0)   #一次多少张图片
        ...
        ]   #先由【b,c,h,w】-[b,h,w,c] 展平成【-1，80】，【-1，4】，就是将各层的全部加在一起
        flatten_cls_scores = torch.cat(flatten_cls_scores)
        flatten_bbox_preds = torch.cat(flatten_bbox_preds)
        flatten_bbox_preds_refine = torch.cat(flatten_bbox_preds_refine)
        flatten_labels = torch.cat(labels)
        flatten_bbox_targets = torch.cat(bbox_targets)  #【l,t,r,b】
        # repeat points to align with bbox_preds  将样本点复制目标数量
        flatten_points = torch.cat(
            [points.repeat(num_imgs, 1) for points in all_level_points])

        # FG cat_id: [0, num_classes - 1], BG cat_id: num_classes
        bg_class_ind = self.num_classes   #背景为80
        pos_inds = torch.where(
            ((flatten_labels >= 0) & (flatten_labels < bg_class_ind)) > 0)[0] #返回0《= 《80 ，TRUE的索引
        num_pos = len(pos_inds)   #被定义成存在目标的数量
        #正样本POS
        pos_bbox_preds = flatten_bbox_preds[pos_inds]   #将预测的匹配上LABEL存在的地方
        pos_bbox_preds_refine = flatten_bbox_preds_refine[pos_inds]
        pos_labels = flatten_labels[pos_inds] #    #这里梁阶段用的是同样的政府样本

        # sync num_pos across all gpus 在所有gpu上同步num pos
        ...
        pos_decoded_bbox_preds = self.bbox_coder.decode(  #【l,t,r,b】解码成【X1，Y1，X2，Y2】
            pos_points, pos_bbox_preds)
        pos_decoded_target_preds = self.bbox_coder.decode( #
            pos_points, pos_bbox_targets)  #
        iou_targets_ini = bbox_overlaps(   #解码成预测的BOX后和TARGET计算IOU，用来干嘛？
            pos_decoded_bbox_preds,
            pos_decoded_target_preds.detach(),
            is_aligned=True).clamp(min=1e-6)
        bbox_weights_ini = iou_targets_ini.clone().detach() #计算GIOU是药用
        bbox_avg_factor_ini = reduce_mean(
            bbox_weights_ini.sum()).clamp_(min=1).item()  #全部IOU加起来再求平均值，计算LOSS的时候用
        ...
        if num_pos > 0:
            loss_bbox = self.loss_bbox(       #计算LOSS，不是计算【l,t,r,b】，而是GIOU
                pos_decoded_bbox_preds,
                pos_decoded_target_preds.detach(),  #两个阶段的样本都是一样的
                weight=bbox_weights_ini,
                avg_factor=bbox_avg_factor_ini)   #GIOU计算要甬道
            ...
            if self.use_vfl:
                ...
                cls_iou_targets[pos_inds, pos_labels] = pos_ious  #将对应位置的LABEL改称相应的IOU
        else:
    ...
    def get_anchors(self, featmap_sizes, img_metas, device='cuda'):
        ...
        """
        num_imgs = len(img_metas)   #两张图片

        # since feature map sizes of all images are the same, we only compute
        # anchors for one time
        multi_level_anchors = self.atss_prior_generator.grid_priors(
            featmap_sizes, device=device)          #返回一个尺度的multi_level_anchors
    ...
    def get_atss_targets(self,
        ...
        anchor_list, valid_flag_list = self.get_anchors(
            featmap_sizes, img_metas, device=device)   #返回设置的anchor_list, valid_flag_list
        ...
        cls_reg_targets = ATSSHead.get_targets(
            ...
            unmap_outputs=True)    #返回的reg_targets是对应原图的。并且是5个特征层上
        if cls_reg_targets is None:
            return None

        (anchor_list, labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, num_total_pos, num_total_neg) = cls_reg_targets  #将元组里的值一一对应赋值
