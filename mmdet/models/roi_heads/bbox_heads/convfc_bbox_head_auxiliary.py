...
class AuxiliaryConvFCBBoxHead(AuxiliaryBBoxHead):
    ...
    def forward(self, x):
        ...
        cls_score = self.fc_cls(x_cls) if self.with_cls else None  #【，81】#两个全连接层
        bbox_pred = self.fc_reg(x_reg) if self.with_reg else None   #【，81*4】
        return cls_score, bbox_pred


@HEADS.register_module
class AuxiliarySharedFCBBoxHead(AuxiliaryConvFCBBoxHead):

    def __init__(self, num_fcs=2, fc_out_channels=1024, *args, **kwargs):
        assert num_fcs >= 1
        super(AuxiliarySharedFCBBoxHead, self).__init__(
            num_shared_convs=0,
