...
# from mmdet.models.utils.inverted_residualcls import InvertedResidual  #这个在新版本已经有了
# from mmcls.models.utils import SELayer, make_divisible
from mmdet.models.utils.inverted_residualcls import InvertedResidual
from mmdet.models.utils.se_layercls import SELayer
...
def model_scaling(layer_setting, arch_setting):
    ...
    for layer_cfg in new_layer_setting:
        for block_cfg in layer_cfg:
            block_cfg[1] = make_divisible(block_cfg[1] * arch_setting[0], 8)   # 使得通道数能被8整除
    ...
    for layer_cfg in new_layer_setting[1:-1]:
        ...
        for i in range(len(layer_cfg) - 1):
            if layer_cfg[i + 1][1] != layer_cfg[i][1]:   # ！= 3
    ...
    new_layers = [
        int(math.ceil(arch_setting[1] * num)) for num in num_of_layers  # 每个BLOCK循环的次数
    ]
