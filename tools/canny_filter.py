...
def get_sobel_kernel(k=3):
    ...
    sobel_2D_numerator = x  #分子
    sobel_2D_denominator = (x ** 2 + y ** 2)  #分母
...
class CannyFilter(nn.Module):
    def __init__(self,
        ...
        # a.data是不参与requires_grad的,如果一个变量要计算梯度，就不要修改它。
        # 你要修改就把requires_grad设置为false。等修改好了在设置requires_grad为true。 这样不会就不会影响计算梯度。
        # print(a)  # requires_grad=True
        # print(a.data.requires_grad)  # False

        # sobel
        sobel_2D = get_sobel_kernel(k_sobel)
        self.sobel_filter_x = nn.Conv2d(in_channels=inchannel,
        ...
        self.sobel_filter_y = nn.Conv2d(in_channels=inchannel,
                                        out_channels=outchannel,
                                        kernel_size=k_sobel,
                                        padding=k_sobel // 2,
                                        bias=False)
