...
if __name__ == '__main__':
    ...
    data_json_raw = json.load(open("/data/wjh/datasets/forUser_A/train/annotations/trainall.json", "r"))  # gt box
    # data_json = json.load(open("/data/wjh/mmdetection-2.19.0/work_dirs/cascade_rcnn_r50_rfp_sac_iou_e15_alldata_v3/submit0.bbox.json", "r"))  # pred box
    data_json = json.load(open("/data/wjh/mmdetection-2.19.0/work_dirs/swa_cascade_rcnn_r50_rfp_sac_iou_alldata-v3_e15/submit_all.bbox.json","r"))
    img = data_json_raw['images']

    unclear_anno_img = []  # 看不清的图片，自己记录

    images = []
    gt_imgid2anno = {}    # {id:真实图像的box}
    pred_imgid2anno = {}  # {id:预测图像的box}
    imgid2name = {}       # {id:图像名}
    for imageinfo in data_json_raw['images']:  # 真实标注的image name
        imgid = imageinfo['id']     # 图像ID
        imgid2name[imgid] = imageinfo['file_name']  # 根据ID找图像名
    # print(len(imgid2name))  # 7600

    for anno in data_json_raw['annotations']:  # 真实标签
        img_id = anno['image_id']
        if img_id not in gt_imgid2anno:
            gt_imgid2anno[img_id] = []      # 生成一系列[]
        gt_imgid2anno[img_id].append(anno)  # 每个图像对应一个标签
    ...
    for imgid in tqdm(gt_imgid2anno.keys()):
        ...
        ious = bbox_iou(pred_boxes[:, :4], gt_boxes[:, :4])  # [n, k],计算IOU
        max_idx = np.argmax(ious, axis=0)  # [k,],返回最大索引
        max_value = np.amax(ious, axis=0)  # [k,],返回最大IOU值
        pred_boxes = pred_boxes[max_idx]   # gt 对应的 pred box,预测的BOX根据索引取出

        flag = False
        diff_boxes = []  # 可能标注错误的box
        for i in range(len(gt_boxes)):
            if gt_boxes[i][4] != int(pred_boxes[i][5]) and max_value[i] >= 0.6: # 如果预测的类别不同GT且IOU>0.6
                # print(imgid2name[imgid])
                diff_boxes.append(pred_boxes[i])    # 保存预测的
        ...
        if flag:
            ...
            # print(imgid2name[imgid])
            filename = os.path.join('/home/yzy/datasets/underwater/forUser_A/train/val-image', imgid2name[imgid])
            img = cv2.imread(filename)
    ...
    # 划分数据集， 10%作为验证集
    np.random.shuffle(revised_annos)    # 随机大乱,但是可以复现
    ...
    generate_json('/data/wjh/datasets/forUser_A/train/image', revised_annos,
                  '/data/wjh/datasets/forUser_A/train/annotations/trainall-revised-v1.json')
    print("train revised data!",len(train_revised_annos))
    generate_json('/data/wjh/datasets/forUser_A/train/image', train_revised_annos,
                  '/data/wjh/datasets/forUser_A/train/annotations/train-revised-v1.json')
    print("val revised data!",len(val_revised_annos))
    generate_json('/data/wjh/datasets/forUser_A/train/image', val_revised_annos,
                  '/data/wjh/datasets/forUser_A/train/annotations/val-revised-v1.json')
        # if flag:
        #     diff_boxes = np.array(diff_boxes)
        #     # print(imgid2name[imgid])
        #     filename = os.path.join('../underwater_data/train/image', imgid2name[imgid])
